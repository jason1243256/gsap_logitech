window.addEventListener('DOMContentLoaded', function () {
  ...
  gsap.to('#section02 .we', {
    scrollTrigger: {
      ...
      start: 'top top', // 앞의 값 : trigger에 적용되어 있는 요소의 위치, 뒤의 값 : 화면의 표시 위치
      end: '109.5% bottom', // 앞의 값 : trigger에 적용되어 있는 요소의 위치, 뒤의 값 : 화면의 표시 위치
      scrub: true,
      pin: true, // trigger에 지정된 요소의 크기만큼 화면을 고정
      // markers: true,
    },
  ...
  sec04.to('#section04 .img06', { y: -3200 }, 0);
  ...
  // section 06 chapter square height 0 animation
  ...
  // section 06 chapter title move animation
  ...
  // section 06 chapter headset animation
  ...
  gsap.to('#section07 .side', {
    scrollTrigger: {
      ...
      onEnterBack: () => {
        //스크te롤이 트리거 요소의 시작 위치에 역방향으로 도달했을 때
        $('#section07 .side').addClass('on');
      },
      onLeave: () => {
      ...
      onLeaveBack: () => {
        //스크롤이 트리거 요소의 끝 위치를 역방향으로 지낫을 때
        $('#section07 .side').removeClass('on');
      }, // 화면 스크롤을 되돌릴때 스크럽 연결 - 없으면 on 클래스가 제거되지 않음
    },
  });
  ...
  //section 091 header remove class
  gsap.to('.fix-this-91', {
    scrollTrigger: {
  ...
  //section 091 mouth image animation
